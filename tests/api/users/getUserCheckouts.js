...
describe('User API, List Users checkouts Endpoint', () =>
{
    before(async () =>
    {
        store.id = await spec()
    ...
    it('should return 200, and list user with id 1 checkouts', async () =>
    {   
        store.data = await spec()
            .withAuth(adminCredentials.username, adminCredentials.password)
            .get(`${user_endpoint}/1/checkouts`)
            .expectStatus(200)
            .expectJsonLike('data', [])
            .returns('data', 'data');
    });
    ...
    it('should return 200, and the return an empty list', async () =>
    {
        store.data = await spec()
            .withAuth(adminCredentials.username, adminCredentials.password)
            .get(`${user_endpoint}/${store.id}/checkouts`)
            .expectStatus(200)
            .expectJsonLike('data', [])
    ...
    it('should return 404, because the user with Id = 10000 does not exist', async () =>
    {
        await spec()
            .withAuth(adminCredentials.username, adminCredentials.password)
            .get(`${user_endpoint}/10000/checkouts`)
            .expectStatus(404);
    });
    ...
    it('should return 400, because the user id is invalid', async () =>
    {
        await spec()
            .withAuth(adminCredentials.username, adminCredentials.password)
            .get(`${user_endpoint}/test/checkouts`)
            .expectStatus(400);
    });
